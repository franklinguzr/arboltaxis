...
public class BtreeT {
    ...
    public BtreeT() {
        ...
        root.key[0] = ""+-1;
        Termino t = new Termino(""+-1,0);
        root.terminos[0]=t ;
    }

    public boolean menorQue(String a, String b){
    boolean retorno= false;
    int x= a.compareToIgnoreCase(b);
     return (x<0);  
    }
     public boolean menorIgual(String a, String b){
    boolean retorno= false;
    int x= a.compareToIgnoreCase(b);
     return (x<=0);  
    }
    public boolean mayorQue(String a, String b){
    boolean retorno= false;
    int x= a.compareToIgnoreCase(b);
     return (x>0);  
    }
    public boolean mayorIgual(String a, String b){
    boolean retorno= false;
    int x= a.compareToIgnoreCase(b);
     return (x>=0);  
    }
    public boolean igualQue(String a, String b){
    boolean retorno= false;
    int x= a.compareToIgnoreCase(b);
     return (x==0);  
    }
     ...
     */
    public void insertar(String a, int b){
        Termino k = new Termino(a,b);  
        // root node r
        Node r = root;
     ...
     */
    public void insertNonfull(Node node, Termino value) {
        // Alocate the node 
        int i = node.numberOfNodes;
        // if the node is a isLeaf, we insert the value into the node 
        if (node.isLeaf) {
            while (i >= 1 && menorQue(value.getValor(), node.terminos[i - 1].getValor() )) {
                node.key[i] = node.key[i - 1];
                node.terminos[i]=node.terminos[i-1];
                i--;
            }
            node.terminos[i]=value;
            node.key[i] = value.getValor();
            node.numberOfNodes++;
        } /*
         ...
         */ else {
            while (i >= 1 && menorQue( value.valor , node.terminos[i - 1].getValor() )) {
                i--;
            }
            i++;
            ...
            if (node.children[i - 1].numberOfNodes == 3) {
                ...
                if (menorQue(node.terminos[i - 1].getValor(),value.getValor() )) {
                    i++;
                }
            }
            // Recursive Call
    ...
    public void splitChild(Node parentNode, int childIndex, Node newChild) {
        ...
        z.terminos[0]= newChild.terminos[2];
        ...
        for (int j = parentNode.numberOfNodes + 1; j >= childIndex + 1; j--) {
            ...
            parentNode.terminos[j - 1]= parentNode.terminos[j - 2];
        }
        ...
        parentNode.terminos[childIndex - 1] = newChild.terminos[1];
        // increment
     ...
     */
    public int search(Node node, String value) {
        // initial i to be equal to 1
        int i = 1;
        // find the smallest index i such that k<= x.keyi, or else set i to x.n ++
        while ((i <= node.numberOfNodes )&& (mayorQue(value , node.terminos[i - 1].getValor()))) {
            //increment i
            i++;
        }
        // check if the key k is found
        if (i <= node.numberOfNodes && igualQue(value, node.terminos[i - 1].getValor())) {
            // return true if the key k is found in the B Tree
            
            return node.terminos[i-1].getDireccion();
        }
...
//---
        // find the smallest index i such that k<= x.keyi, or else set i to x.n ++
        while (i <= node.numberOfNodes && mayorQue(value+"" , node.key[i - 1])) {
            //increment i
            i++;
        }
        /* if the x node is an internal node, terminate the search unsuccessful or 
        ...
        if (node.isLeaf) {
            // recursive call to search in the subtree of the node x
            // check if the key k is found
            if (i <= node.numberOfNodes && igualQue(value+"" , node.key[i - 1])) {
                // return true if the key k is found in the B Tree
                node.key[i - 1] = 0+"";
